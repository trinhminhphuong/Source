      ...
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "flex-start" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 8 }}>{product.brand}</p>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              <div style={{ display: "flex" }}>
                {[1,2,3,4,5].map(s => (
                  <Star key={s} size={16} fill={s <= Math.round(product.rating) ? "var(--pk-gold)" : "none"} color={s <= Math.round(product.rating) ? "var(--pk-gold)" : "#e0c4d0"} />
                ))}
              </div>
              <span style={{ fontWeight: 700, fontSize: "0.9rem" }}>{product.rating}</span>
              <span style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>({product.reviews} đánh giá)</span>
            </div>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
              ...
              <button onClick={() => { toggle(product.id); show(wishlisted ? "Đã xoá khỏi yêu thích" : "Đã thêm vào yêu thích!"); }}
