      ...
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 32, alignItems: "flex-start" }}>
            ...
            {items.map(item => (
              <div key={item.id} className="card" style={{ padding: 18, display: "flex", gap: 16, alignItems: "center" }}>
                <Link to={`/products/${item.id}`}>
                  <img src={item.image} alt={item.name} style={{ width: 90, height: 90, objectFit: "cover", borderRadius: "var(--radius-md)", flexShrink: 0 }} />
                </Link>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: "0.7rem", color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{item.brand}</p>
                  <Link to={`/products/${item.id}`}>
                    <h3 style={{ fontWeight: 600, fontSize: "0.95rem", marginBottom: 6, color: "var(--text)" }}>{item.name}</h3>
                  </Link>
                  <p style={{ color: "var(--pk-pink)", fontWeight: 700 }}>{formatPrice(item.price)}</p>
                </div>
                {/* Qty */}
                ...
                <div style={{ textAlign: "right", minWidth: 100 }}>
                  <p style={{ fontWeight: 700, color: "var(--text)" }}>{formatPrice(item.price * item.qty)}</p>
                </div>
                {/* Remove */}
          ...
          <div className="card" style={{ padding: 24, width: 300, position: "sticky", top: 90 }}>
            ...
            <Link to="/checkout" className="btn-primary" style={{ display: "flex", width: "100%", justifyContent: "center", marginBottom: 10 }}>
